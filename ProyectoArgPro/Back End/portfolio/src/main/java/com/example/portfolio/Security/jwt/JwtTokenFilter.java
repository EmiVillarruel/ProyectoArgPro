...
import com.example.portfolio.Security.Service.UserDetailsImpl;
...
public class JwtTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            if (token != null && jwtProvider.validaToken(token)) {
                String nombreUsuario = jwtProvider.getNombreUsuarioFromTken(token);
    ...
    private String getToken(HttpServletRequest request){
        String header = request.getHeader("Authotizacion");
